...
-- // GLOBAL STATES (Preserving everything)
_G.espActive = false
...
-- // STRICT API FOLLOW CHECKER
local function checkFollowStatus()
    local success, result = pcall(function()
        -- Checking if the LocalPlayer is following the target ID
...
-- // MAIN CHEAT MENU (Full Integration)
function StartCheatMenu()
    ...
    local function AddBtn(txt, pg, cb)
        ...
        btn.MouseButton1Click:Connect(function() cb(btn) 
            local active = (_G.espActive and txt == "Toggle ESP") or (_G.afkFarmActive and txt == "Autofarm (7s)") or (_G.noclipActive and txt == "Noclip") or (_G.floatActive and txt == "Float Platform") or (_G.fullbrightActive and txt == "Fullbright") or (_G.zoomUnlocked and txt == "Unlock Zoom")
    ...
    CreateTab("Main", 1); CreateTab("Player", 2); CreateTab("Visuals", 3); CreateTab("Teleport", 4)
    AddLabel("🏠 YOU VS HOMER SCRIPT v3.7.4", "Main", Color3.new(1, 1, 0))
    AddLabel("Status: Verified (Follower Check Passed)", "Main", Color3.new(0, 1, 0))
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if _G.espActive then for _, v in pairs(Players:GetPlayers()) do if v ~= player and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then local role, color = getRoleInfo(v) local h = v.Character:FindFirstChild("HHubESP") or Instance.new("Highlight", v.Character) h.Name = "HHubESP"; h.FillColor = color; h.DepthMode = "AlwaysOnTop" local head = v.Character:FindFirstChild("Head") if head and not v.Character:FindFirstChild("HHubTag") then local tag = Instance.new("BillboardGui", v.Character); tag.Name = "HHubTag"; tag.Size = UDim2.new(0, 200, 0, 50); tag.AlwaysOnTop = true; tag.Adornee = head; tag.ExtentsOffset = Vector3.new(0, 3, 0) local lbl = Instance.new("TextLabel", tag); lbl.Size = UDim2.new(1, 0, 1, 0); lbl.BackgroundTransparency = 1; lbl.Text = role .. " | " .. v.Name; lbl.TextColor3 = color; lbl.Font = "GothamBold"; lbl.TextSize = 16 end end end end
    end)
    task.spawn(function() while true do task.wait(7) if _G.afkFarmActive and player.Character then local w = Workspace:FindFirstChild("winpad", true) if w then local old = player.Character.HumanoidRootPart.CFrame player.Character.HumanoidRootPart.CFrame = w.CFrame * CFrame.new(0, 4, 0) task.wait(0.2); player.Character.HumanoidRootPart.CFrame = old end end end end)
end

-- // VERIFICATION GATE
function StartVerification()
    local sg = Instance.new("ScreenGui", game.CoreGui)
    sg.Name = "HomerVerifyGui"
    
    local vMain = Instance.new("Frame", sg)
    vMain.Size = UDim2.new(0, 400, 0, 0); vMain.Position = UDim2.new(0.5, -200, 0.5, -125)
    vMain.BackgroundColor3 = Color3.fromRGB(15, 15, 15); vMain.BackgroundTransparency = 0.2
    vMain.ClipsDescendants = true; Instance.new("UICorner", vMain).CornerRadius = UDim.new(0, 15)

    local title = Instance.new("TextLabel", vMain)
    title.Size = UDim2.new(1, 0, 0, 50); title.Position = UDim2.new(0, 0, 0, 10); title.Text = "STRICT VERIFICATION"; title.TextColor3 = Color3.new(1, 1, 0); title.Font = "GothamBold"; title.TextSize = 20; title.BackgroundTransparency = 1

    local desc = Instance.new("TextLabel", vMain)
    desc.Size = UDim2.new(1, -40, 0, 60); desc.Position = UDim2.new(0, 20, 0, 60); desc.Text = "Please follow derWolfderwutet. Verification checks your live followings list."; desc.TextColor3 = Color3.new(1,1,1); desc.Font = "Gotham"; desc.TextSize = 15; desc.TextWrapped = true; desc.BackgroundTransparency = 1

    local copyBtn = Instance.new("TextButton", vMain)
    copyBtn.Size = UDim2.new(0, 320, 0, 45); copyBtn.Position = UDim2.new(0.5, -160, 0, 130); copyBtn.BackgroundColor3 = Color3.fromRGB(40, 40, 40); copyBtn.Text = "COPY PROFILE LINK"; copyBtn.TextColor3 = Color3.new(1,1,1); copyBtn.Font = "GothamBold"; copyBtn.TextSize = 17; Instance.new("UICorner", copyBtn)

    local verifyBtn = Instance.new("TextButton", vMain)
    verifyBtn.Size = UDim2.new(0, 320, 0, 45); verifyBtn.Position = UDim2.new(0.5, -160, 0, 185); verifyBtn.BackgroundColor3 = Color3.fromRGB(50, 200, 50); verifyBtn.Text = "VERIFY FOLLOW"; verifyBtn.TextColor3 = Color3.new(1,1,1); verifyBtn.Font = "GothamBold"; verifyBtn.TextSize = 17; Instance.new("UICorner", verifyBtn)
    ...
    verifyBtn.MouseButton1Click:Connect(function()
        ...
        if isFollowing then
            verifyBtn.Text = "SUCCESS!"; verifyBtn.BackgroundColor3 = Color3.fromRGB(0, 255, 100)
            task.wait(0.5)
            ...
            local op = vMain.Position
            for i = 1, 8 do vMain.Position = op + UDim2.new(0, math.random(-6, 6), 0, 0) task.wait(0.04) end
            vMain.Position = op
            task.wait(2)
            verifyBtn.Text = "VERIFY FOLLOW"; verifyBtn.BackgroundColor3 = Color3.fromRGB(50, 200, 50)
        end
    end)
